...
export class HttpClientError extends Error {
  ...
  constructor(
    message: string,
    options: { status?: number; detail?: string; isTimeout?: boolean; isNetworkError?: boolean } = {},
  ) {
    super(message);
...
async function toHttpClientError(response: Response): Promise<HttpClientError> {
  ...
  return new HttpClientError(`HTTP ${response.status}: ${detail || response.statusText}`, {
      ...
      const normalizedError =
        error instanceof HttpClientError
          ...
          : error instanceof Error && error.name === 'AbortError'
            ? new HttpClientError(`Request timeout after ${httpConfig.timeoutMs}ms: ${url}`, { isTimeout: true })
            : new HttpClientError(`Network request failed: ${error instanceof Error ? error.message : String(error)}`, {
                isNetworkError: true,
              });
